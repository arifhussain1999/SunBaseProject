...
async function removeRow(id) {
    ...
    try {
        ...
        const res = await fetch(`http://localhost:8080/customer/delete-customer?id=${id}`, {
            method: 'DELETE',
            headers: {
...
async function addUser(user) {
    try {
        const token = sessionStorage.getItem("token");
        const res = await fetch(`http://localhost:8080/customer/add-customer`, {
            method: "POST",
            headers: {
                "Authorization": `Bearer ${token}`,
                'Content-Type': 'application/json',
...
async function editUser(id) {
    const token = sessionStorage.getItem("token");
    const res = await fetch(`http://localhost:8080/customer/get-customer?id=${id}`, {
        method: "GET",
        headers: {
            "Authorization": `Bearer ${token}`,
            'Content-Type': 'application/json',
